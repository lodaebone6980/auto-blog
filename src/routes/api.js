...
function qualityExpansionParagraph({ keyword = '', topic = '', index = 0, includeKeyword = true } = {}) {
  ...
  const keyObject = `${key}${objectParticle(key)}`;
  const keyTopic = `${key}${koreanParticle(key, '은', '는')}`;
  const variants = [
    `${keyObject} 확인할 때는 제목에 보이는 문구만 보지 말고 실제 공고명, 기준일, 접수처가 서로 맞는지 함께 보는 편이 안전합니다. 비슷한 안내가 여러 곳에 올라와도 최종 판단은 공식 화면에서 현재 열려 있는 메뉴와 공지 날짜를 대조한 뒤 진행하는 것이 좋습니다.`,
    ...
    `${keyTopic} 지역이나 운영 주체에 따라 세부 표현이 달라질 수 있습니다. 따라서 같은 주제의 안내라도 금액, 기간, 신청 경로처럼 바뀔 수 있는 값은 최신 공지를 기준으로 다시 확인하고, 변동 가능성이 낮은 준비 순서만 먼저 챙기는 방식이 안정적입니다.`,
  ];
